...
public enum MissionType {
    ...
    PLAY_GAME("SELECT COUNT(r) FROM User_friend ur WHERE FUNCTION('DATE', ur.createdTime) = CURRENT_DATE AND ur.user.id = :userId"),
    ATTEND_STUDY("SELECT COUNT(r) FROM Review r WHERE FUNCTION('DATE', r.createdTime) = CURRENT_DATE AND r.user.id = :userId"),
